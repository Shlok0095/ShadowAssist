...
/** Deepgram default for fast turn detection (was 220ms — added ~200ms+ vs NVIDIA). */
const DEFAULT_ENDPOINTING_MS = 10
...
class DeepgramChannel {
  ...
  async flushAndWait(timeoutMs = 700) {
...
function getConfig() {
  ...
  const endpointing =
    Number.isFinite(endpointingRaw) && endpointingRaw >= 10 && endpointingRaw <= 500
...
function buildListenParams(cfg) {
  const params = new URLSearchParams({
    ...
    endpointing: String(cfg.endpointing ?? DEFAULT_ENDPOINTING_MS),
    vad_events: 'true',
    no_delay: 'true',
  })
