...
// Deepgram live streaming STT — WebSocket in main process (Natively-aligned).
...
class DeepgramChannel {
  ...
  async connect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Clear the keepalive on error/close too — otherwise every reconnect
      // leaks an 8s interval that pings a dead socket for the session lifetime.
  ...
  async flushAndWait(timeoutMs = 450) {
...
function getConfig() {
  ...
  if (!key) return null
  return { apiKey: key, model: storeGet('deepgramModel') || 'nova-2' }
}

async function ensureChannel(ch) {
  const key = ch === 'sys' ? 'sys' : 'mic'
  if (channels[key]?.active) return channels[key]
  const inst = new DeepgramChannel(key, getConfig, (text) => {
...
function ensureRestChannel(ch) {
  ...
  const inst = new DeepgramRestChannel(key, getConfig, (text) => {
    if (transcriptCallback) transcriptCallback({ channel: key, text, isFinal: true })
  })
  channels[key] = inst
  return inst
