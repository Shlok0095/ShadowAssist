...
const schema = {
  /** Chat / Ask AI provider — nvidia, openai, anthropic, google, groq, deepseek, custom */
...
/** Chat LLM moves off Groq → NVIDIA multimodal once; STT stays independent. */
function migrateChatProviderGroqToNvidia() {
  try {
    if (get('chatProviderMigratedToNvidiaV1') === true) {
      migrateDeprecatedNvidiaChatModels()
      return
    }
    const chatProv = String(get('provider') || '').trim()
    if (chatProv === 'groq' || !chatProv) {
      const sttProv = String(get('sttProvider') || '').trim()
      if (!sttProv) set('sttProvider', 'groq')
      set('provider', 'nvidia')
    }
    const { isMultimodalChatModel } = require('./chatMultimodalModels')
    const { resolveNvidiaChatModel, DEFAULT_NVIDIA_CHAT_MODEL } = require('./nvidiaChatModels.cjs')
    const nvidiaModel = String(get('nvidiaModel') || '').trim()
    if (!nvidiaModel || !isMultimodalChatModel('nvidia', nvidiaModel)) {
      set('nvidiaModel', DEFAULT_NVIDIA_CHAT_MODEL)
    } else {
      set('nvidiaModel', resolveNvidiaChatModel(nvidiaModel))
    }
    set('chatProviderMigratedToNvidiaV1', true)
  } catch (_) {}
}
...
function runDataMigration() {
  ...
  migrateChatProviderGroqToNvidia()
  migrateDeprecatedNvidiaChatModels()
