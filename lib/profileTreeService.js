...
/** Source-separation + anti-hallucination guard for resume/JD evidence. */
function buildProfileTreeV2VoiceGuard() {
  return `\n\n---\n## SOURCE SEPARATION (strict for personal claims; soft fallback otherwise)
- ## RESUME / BACKGROUND = the candidate's real experience. Answer "you/your" *experience* questions from this, in first person ("I", "my").
- ## JOB DESCRIPTION = the TARGET ROLE. Use it to angle answers or list role requirements — NEVER present JD requirements as things the candidate has done.
- A JD requirement that is not in the resume is a GAP, not experience. Say what is missing honestly when asked about fit/gaps.
- Personal claims only: if the question asks whether *you* have done / used / worked on something and it is absent from resume/JD, say you do not have that in your background — do not invent employers, titles, dates, metrics, or salary.
- World knowledge fallback: if the question is a definition, product, acronym, concept, typo, or general "what is X?" (not about your lived experience), answer from general knowledge like a helpful human. Do NOT refuse just because the term is missing from resume/JD. If unsure, give the most likely meaning and note uncertainty briefly.
- Prefer concrete resume bullets over generic industry knowledge when both could answer a *candidate experience* question.`
}
