...
/** All NIM chat/vision models we use — disable extended CoT for overlay latency. */
function isNvidiaFastChatModel(baseURL, model) {
  if (!isNvidiaNimHost(baseURL)) return false
  return /nemotron|mistral-small-4|llama-3\.2-.*vision|llama-3\.1-nemotron/i.test(String(model || ''))
}

function applyProviderReasoningControls(messages, baseURL, model) {
  if (!isNvidiaFastChatModel(baseURL, model)) return messages
    ...
    : qwenGroqVision
      ...
      : nvidiaFastVision
        ? {
            ...
            top_p: 0.7,
            seed: 7,
            // Disables chain-of-thought on Nemotron-family NIM templates when supported.
