...
const REGISTRY = {
  ...
  nvidia: {
    ...
    ui: {
      ...
      badge: 'NIM',
      color: '#a78bfa',
      desc: 'Nemotron VL + Mistral Small — screen + transcript',
...
function getProviderMetadataForUI() {
  // Chat/Ask providers only — Groq remains STT-only (Audio settings).
  return ORDER.filter((id) => REGISTRY[id] && id !== 'groq').map((id) => {
    const e = REGISTRY[id]
    return {
      id,
      kind: e.kind,
