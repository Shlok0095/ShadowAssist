...
function finalizeModelList(apiIds, staticList, savedModel, provider) {
  ...
  if (apiIds?.length) {
    ...
    const merged = filterMultimodalChatModels(provider, out)
    if (provider === 'openrouter') {
      return prioritizeOpenRouterModels(merged, staticFiltered)
    }
    return merged
  }
...
function prioritizeOpenRouterModels(ids, staticList) {
  const preferred = mergeUnique(staticList, [
    'meta-llama/llama-4-scout',
    'nvidia/nemotron-nano-12b-v2-vl:free',
    'google/gemma-4-26b-a4b-it:free',
    'google/gemma-4-31b-it:free',
  ])
  const free = (ids || []).filter((id) => String(id).endsWith(':free'))
  const rest = (ids || []).filter((id) => !String(id).endsWith(':free'))
  return mergeUnique(preferred, free, rest)
}
...
async function fetchOpenRouterModelList() {
  ...
  const j = await r.json()
  const ids = (j.data || [])
    .filter((m) => m?.id && isLikelyChatModelId(m.id) && isOpenRouterVisionModel(m))
    .map((m) => m.id)
  if (!ids.length) throw new Error('Empty OpenRouter vision model list')
  return ids
}

/** OpenRouter architecture: modality string ("text+image->text") or input_modalities array. */
function isOpenRouterVisionModel(model) {
  const arch = model?.architecture || {}
  if (typeof arch.modality === 'string' && /image/i.test(arch.modality)) return true
  const inputs = arch.input_modalities || arch.modality_type || []
  if (Array.isArray(inputs) && inputs.some((x) => /image/i.test(String(x)))) return true
  return false
}
