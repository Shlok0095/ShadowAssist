...
class StreamingSession {
  ...
  stop() {
    ...
    for (const seg of finals) {
      void this.dispatchFinal(seg.samples)
    }
  ...
  async stopAndDrain() {
    ...
    const finals = this.vad.flush()
    const tasks = finals.map((seg) => this.dispatchFinal(seg.samples))
  ...
  writePcm(pcm, sampleRate) {
    ...
    if (open && open.durationMs >= MAX_SEGMENT_MS) {
      ...
      if (committed) void this.dispatchFinal(committed.samples)
    }
  ...
  async dispatchFinal(samples) {
    ...
    while (this.streamingTaskInFlight && this.active && Date.now() - waitStart < 15000) {
    ...
    try {
      ...
      this.agreement.handleFinal(cleaned, (text) => this.onFinal(text))
    } catch (err) {
