...
public static class SaVisibleMeetingScan {
  ...
  static bool LooksLikeMeet(string t) {
    ...
    if (s.IndexOf("google meet", StringComparison.OrdinalIgnoreCase) >= 0) return true;
  ...
  static bool IsBrowserExe(string exe) {
    ...
    switch (exe.ToLowerInvariant()) {
      ...
      case "chrome": case "msedge": case "brave": case "opera": case "vivaldi":
