...
function createHindsightClient({
  ...
  async function hybridRecall(input = {}) {
    ...
    const localPromise = (async () => {
      ...
      const useVector =
        input.useVectorMemory !== false &&
        vectorMemory?.isEnabled?.() &&
    ...
    const externalUrl = String(store.get('hindsightApiUrl') || process.env.HINDSIGHT_API_URL || '').trim()
  ...
  return { hybridRecall, searchTranscriptLines }
}
